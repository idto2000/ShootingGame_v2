...
namespace Wanted
{
	...
	Engine::Engine()
	{
		...
		// 버퍼 사이즈 적용
		COORD coord;                      // 구조체 생성
		coord.X = (short)setting.width;   // 첫 번째 변수(X)에 값 넣기
		coord.Y = (short)setting.height;  // 두 번째 변수(Y)에 값 넣기

		SMALL_RECT rect;
		rect.Left = 0;                  // 왼쪽 위 끝점 X 좌표
		rect.Top = 0;                   // 왼쪽 위 끝점 Y 좌표
		rect.Right = coord.X - 1;       // 오른쪽 아래 끝점 X 좌표
		rect.Bottom = coord.Y - 1;      // 오른쪽 아래 끝점 Y 좌표
